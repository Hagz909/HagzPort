...
export default function LoginPage() {
  ...
  useEffect(() => {
    const emailPlaceholder = "Isi email Anda...";
    let i = 0;
    const emailInput = document.getElementById('email-input');
    if (!emailInput) return;
    emailInput.setAttribute('placeholder', '');
    const interval = setInterval(() => {
      if (i <= emailPlaceholder.length) {
        emailInput.setAttribute('placeholder', emailPlaceholder.slice(0, i));
        i++;
      } else {
      ...
      <div className="w-full max-w-6xl mx-auto flex flex-col-reverse lg:flex-row lg:items-start items-center gap-12 lg:gap-24 relative z-10 animate-cardEntrance">
        ...
        <div className="w-full lg:w-1/2 flex justify-center lg:justify-end">
          ...
          >
            ...
            <form className="space-y-6 relative z-10" onSubmit={handleSubmit(onSubmit)} suppressHydrationWarning>
              <div>
                <label className={`block text-sm font-medium prompt-regular transition-colors ${isDark ? 'text-zinc-300' : 'text-slate-700'}`}>
                  Email
                </label>
                <div className="mt-1">
                  <input
            ...
            <p className={`mt-8 text-center text-sm transition-colors ${isDark ? 'text-zinc-400' : 'text-slate-500'}`}>
              Belum punya akun?{' '}
              <Link
                href="/register"
